...
def producer_process(device_id, shm_name, buffer_size, num_transfers, data_size):
    try:
        ...
        total_data = (data_size * 4 * num_transfers) / (1024**2)  # MB
        ...
        print(f"  Throughput (excl init): {total_data / total_transfer_time:.2f} MB/s")
        print(f"  Throughput (incl init): {total_data / total_time:.2f} MB/s")
...
def consumer_process(device_id, shm_name, num_transfers, expected_size):
    try:
        ...
        print(f" [Consumer] Throughput (excl init): {total_data / total_transfer_time:.2f} MB/s")
        print(f" [Consumer] Throughput (incl init): {total_data / total_time:.2f} MB/s")

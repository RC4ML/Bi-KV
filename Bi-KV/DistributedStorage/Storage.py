...
class LFUCache:
    ...
    def profile(self):
        # print(f"key_to_freq = {self.key_to_freq}")
            # 1. 按键排序后的 key_to_freq 打印
        sorted_key_to_freq = dict(sorted(self.key_to_freq.items(), key=lambda item: item[0]))
        print("Sorted key_to_freq:")
        for key, freq in sorted_key_to_freq.items():
...
class KVCache:
    def __init__(self, total_capacity: int, user_cache_ratio: float, model_layers: int, vector_dim: int):
        ...
        self.item_cache = LRUCache(self.item_capacity)
